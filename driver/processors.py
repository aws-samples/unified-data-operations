...
def freshness_validator(df: DataFrame, col_name: str, cfg: any = None):
    ...
    if hasattr(cfg, "group_by"):
        ...
        res_df = (
            df.select(col(col_name), col(cfg.group_by))
            .withColumn("rn", row_number().over(Window.partitionBy(cfg.group_by).orderBy(col(col_name).desc())))
            .filter(col("rn") == 1)
            .drop("rn")
        )
        ...
        for mismatched_field in mismatched_fields or []:
            ...
            if field_in_df:
                ds.df = ds.df.withColumn(
                    mismatched_field.name, col(mismatched_field.name).cast(mismatched_field.dataType.typeName())
                )
        return ds
    ...
    if test_property(ds, "model.columns"):
        ...
        if hasattr(ds, "model") and hasattr(ds.model, "validation") and ds.model.validation == "strict":
            if not hasattr(ds, "df"):
                raise SchemaValidationException(
                    f"The dataset [{ds.id}] is missing a dataframe with strict validation", ds
                )
...
def constraint_processor(ds: DataSet):
    if not test_property(ds, "model.columns"):
        return ds
...
def transformer_processor(data_set: DataSet):
    ...
    """
    if not test_property(data_set, "model.columns"):
        return data_set
