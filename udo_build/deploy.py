...
def deploy_data_product():
    try:
        ...
        print(f'Putting DAG file {dag_file.key} to {dag_file.bucket_name}.')
        dag_file.put(Body=rendered_dag_content.encode(
        ), ExpectedBucketOwner=sts.get_caller_identity().get('Account'))

        logger.info({'message': 'DAG has been created and deployed successfully'})
    except Exception as e:
        logger.exception({'message': 'DAG creation has failed'})
        raise e
